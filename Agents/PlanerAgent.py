...
def build_trip(tripRequestGraph: Graph):
    ...
    activitiesSearch = Process(target=search_activities, args=(city, festive, cultural, playful, budget, start, end, activitiesGraph))
    ...
    if hotelsSortedPrice[0].get("price")>budget/4:
        ...
        rangActivitats="1"
    else if hotelsSortedPrice[int(len(hotelsSortedPrice)/2)].get("price")>budget/4:
    ...
    chosenLodgingFlight = sorted(lodgingFlightSearch,key=attrgetter("price"))[0]
...
def search_hotels(city=None, location=None, budget=None, hotelsGraph):
    global mss_cnt
...
def search_activities(city, festive, cultural, playful, budget, start, end, activitiesGraph):
    global mss_cnt
    ...
    messageGraph.add((activitiesRequestObj, ONTO.date, Literal(date)))
    messageGraph.add((activitiesRequestObj, ONTO.maxPrice, Literal(budget)))
